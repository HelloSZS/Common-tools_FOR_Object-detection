...
def namelist2txt(label_path, save_path, set = 'train'):
    if os.path.exists(label_path) != True:
        os.makedirs(label_path)
    # else:
    #     print(f"已经存在: {label_path}, 请检查目录")
    #     exit(-1)

    with open(os.path.join(save_path, set + '.txt'), 'w') as f:
        label_list = os.listdir(label_path)
        label_name_list = map(lambda x: x.split('.')[0],label_list)

        for string in label_name_list:
            f.write(string + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ap', '--anno-path', type=str, default='./labels', help='yolo txt path')
    parser.add_argument('-s', '--save-path', type=str, default='./convert_voc', help='xml save path')
    ...
    if len(sys.argv) > 1:
        ...
        for s in Image_set:
            anno_path = f'./labels/{s}'
            save_path = './convert-voc/'
            image_path = f'./images/{s}'
            txt_save_path = './convert-voc/'

            parseXmlFilse(image_path, anno_path, save_path)
            namelist2txt(save_path, txt_save_path, s)
